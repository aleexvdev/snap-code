...
import { BORDERS } from "../constants/framerConstants";
import { setBorder } from "@/redux/features/framerSlice";

export const BorderSelector = () => {
  const [showDropdown, setShowDropdown] = useState<boolean>(false);
  const { border } = useSelector((state: RootState) => state.framer);
      ...
      <div className="w-full flex items-center justify-start">
        ...
        <div className="w-full relative">
          <button
            ...
            onClick={() => setShowDropdown(!showDropdown)}
          >
            <span className="text-sm text-white capitalize">{border}</span>
                  ...
                  {BORDERS.map((item, index) => (
                    <li
                      ...
                      onClick={() => {
                        ...
                        dispatch(setBorder(item.toString()));
                      }}
                    >
                      <span
                        className={`ml-2 text-sm capitalize ${
                          border.toString() === item.toString()
                            ? "text-[#CCCCCC]"
                            : ""
                        }`}
                      >
